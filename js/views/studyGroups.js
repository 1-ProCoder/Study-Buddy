class StudyGroupsView {
    ...
    async render(container) {
        ...
        if (this.useFirebase && userGroups.length > 0) {
            ...
            userGroups.forEach(group => {
                ...
                this.firebaseService.onGroupMessagesUpdate(group.id, (messages) => {
                    const container = document.querySelector(`.group-chat-messages[data-group-id="${group.id}"]`);
                    if (!container) return;
                    if (!messages || messages.length === 0) {
                        container.innerHTML = '<p class="text-muted" style="font-size: 0.85rem; text-align: center; padding: 0.5rem;">No messages yet. Start the conversation!</p>';
                        return;
                    }
                    container.innerHTML = messages.map(msg => `
                        <div style="margin-bottom: 0.35rem;">
                            <div style="font-size: 0.8rem; font-weight: 600; color: var(--text-muted);">${msg.senderName || 'Member'}</div>
            ...
            if (grid) {
                ...
                // Ensure buttons and chat controls work in local mode
    ...
    renderGroupCardSync(group, sharedResources) {
        ...
        const messages = this.getLocalGroupMessages(group.id);
            ...
            <div class="card group-card" data-group-id="${group.id}">
                ...
                <div style="border-top: 1px solid var(--border); padding-top: 1rem; margin-top: 1rem;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                        <h4 style="font-size: 0.95rem; color: var(--text-muted);">Group Chat</h4>
                    </div>
                    <div class="group-chat-messages" data-group-id="${group.id}" style="max-height: 180px; overflow-y: auto; padding: 0.5rem; background: var(--bg-body); border-radius: 8px; border: 1px solid var(--border); margin-bottom: 0.5rem;">
                        ${messages.length === 0 ? `
                            <p class="text-muted" style="font-size: 0.85rem; text-align: center; padding: 0.5rem;">No messages yet. Start the conversation!</p>
                        ` : `
                            ${messages.map(msg => `
                                <div style="margin-bottom: 0.35rem;">
                                    <div style="font-size: 0.8rem; font-weight: 600; color: var(--text-muted);">${msg.senderName || 'Member'}</div>
                    ...
                    <div style="display: flex; gap: 0.5rem; align-items: flex-end;">
                        <textarea id="chat-input-${group.id}" rows="1" placeholder="Type a question or answer..." style="flex: 1; resize: none; padding: 0.5rem 0.75rem; border-radius: 8px; border: 1px solid var(--border); background: var(--bg-body); font-size: 0.9rem;"></textarea>
                        <button class="btn btn-primary send-chat-btn" data-group-id="${group.id}" style="flex-shrink: 0;">Send</button>
                    </div>
                </div>
            </div>
        `;
    ...
    attachGroupCardListeners() {
        ...
        document.querySelectorAll('.send-chat-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                // Build message object using current user
                const currentUser = this.authManager?.getCurrentUser() || this.store.getUser();
                const message = {
                    text,
                    senderId: currentUser?.userId || currentUser?.accountId || 'local',
                    senderName: currentUser?.name || currentUser?.username || 'You',
                    sentAt: new Date().toISOString()
                };

                // Immediately append message to the chat UI without re-rendering
                const container = document.querySelector(`.group-chat-messages[data-group-id="${groupId}"]`);
                if (container) {
                    ...
                    wrapper.innerHTML = `
                        <div style="font-size: 0.8rem; font-weight: 600; color: var(--text-muted);">${message.senderName}</div>
                ...
                // Persist message in background (Firebase or localStorage)
                this.handleSendMessage(groupId, message);
            });
        });
    }
    ...
    getLocalGroupMessages(groupId) {
        try {
            const raw = localStorage.getItem('studybuddy_group_messages');
            if (!raw) return [];
            const data = JSON.parse(raw);
            return Array.isArray(data[groupId]) ? data[groupId] : [];
        } catch (e) {
            console.warn('Error loading local group messages:', e);
            return [];
        }
    }

    saveLocalGroupMessages(groupId, messages) {
        try {
            const raw = localStorage.getItem('studybuddy_group_messages');
            const data = raw ? JSON.parse(raw) : {};
            data[groupId] = messages;
            localStorage.setItem('studybuddy_group_messages', JSON.stringify(data));
        } catch (e) {
            console.warn('Error saving local group messages:', e);
        }
    }

    async handleSendMessage(groupId, message) {
        try {
            if (this.useFirebase) {
                await this.firebaseService.addGroupMessage(groupId, message);
            } else {
                const existing = this.getLocalGroupMessages(groupId);
                existing.push(message);
                this.saveLocalGroupMessages(groupId, existing);
            }
        } catch (e) {
    ...
    async renderGroupCard(group, sharedResources) {
        ...
        if (this.useFirebase) {
            try {
                const messagesResult = await this.firebaseService.getGroupMessages(group.id);
            ...
            messages = this.getLocalGroupMessages(group.id);
        }
    ...
    async handleDeleteGroup(groupId) {
        // Firebase-backed groups
        if (this.useFirebase) {
            await this.firebaseService.deleteStudyGroup(groupId);
            await this.render(document.getElementById('app-view'));
            return;
        }

        // LocalStorage-backed groups
        const groups = this.getGroups();
    ...
    async handleLeaveGroup(groupId) {
        ...
        if (this.useFirebase) {
            ...
            if (!current) return;
            await this.firebaseService.leaveStudyGroup(groupId, current.userId);
            // Re-render to remove group from list
            await this.render(document.getElementById('app-view'));
            return;
        }
        ...
        if (group) {
            const currentAccount = this.authManager?.getCurrentUser();
            const currentUserId = currentAccount?.userId || this.store.getUser()?.userId;
            if (currentUserId) {
                group.members = (group.members || []).filter(m => m.userId !== currentUserId);
                this.saveGroups(groups);
            }
        }
