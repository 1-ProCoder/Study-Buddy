class TimetableView {
    ...
    renderTimetable(timetable) {
        ...
        // Prepare helpers for marking todays subjects as completed once 60+ minutes are logged
        const today = new Date();
        const todayDate = today.toISOString().split('T')[0];
        const weekdayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
        const todayDayName = weekdayNames[today.getDay()];
        const subjects = this.store.getSubjects ? this.store.getSubjects() : [];
        const progress = (this.store.state && this.store.state.timetableStudyProgress) ? (this.store.state.timetableStudyProgress[todayDate] || {}) : {};
        const rewards = (this.store.state && this.store.state.timetableStudyRewards) ? (this.store.state.timetableStudyRewards[todayDate] || {}) : {};

        const isSlotCompleted = (dayName, slot) => {
            if (dayName !== todayDayName) return false;
            const subject = subjects.find(s => s.name === slot.subject);
            if (!subject) return false;
            const subjectId = subject.id;
            if (rewards[subjectId]) return true;
            const mins = progress[subjectId] || 0;
            return mins >= 60;
        };
                        ...
                        <div class="card day-card">
                            ...
                            <div class="day-slots">
                                ${slots.length > 0 ? slots.map(slot => {
                                    const completed = isSlotCompleted(day, slot);
                                    return `
                                        <div class="slot" style="border-left: 6px solid ${slot.color}; ${completed ? 'opacity:0.7; background: var(--bg-body);' : ''}">
                                            <span class="slot-subject" style="${completed ? 'text-decoration: line-through;' : ''}">${slot.subject}</span>
                                            <span class="slot-time">${slot.duration} mins${completed ? ' · ✅' : ''}</span>
                                        </div>
                                    `;
