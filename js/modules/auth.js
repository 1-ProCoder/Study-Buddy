...
class AuthManager {
    ...
    // Permanently delete an account and clean up related local data
    deleteAccount(userId) {
        ...
        // Remove from local leaderboard (sb_all_users)
        try {
            const rawLeaderboard = localStorage.getItem('sb_all_users');
            if (rawLeaderboard) {
                const users = JSON.parse(rawLeaderboard) || [];
                const filtered = users.filter(u => u.userId !== userId);
                localStorage.setItem('sb_all_users', JSON.stringify(filtered));
            }
        } catch (e) {
            console.warn('Error cleaning up leaderboard for deleted account:', e);
        }

        // Remove from local study groups (sb_shared_groups)
        try {
            const rawGroups = localStorage.getItem('sb_shared_groups');
            if (rawGroups) {
            ...
            console.warn('Error cleaning up study groups for deleted account:', e);
        }

        // Clear auth session if it belongs to this user
        const session = this.getSession();
